...
v = 1.0
options = {'maxiter': 200}
while (w_svc[0]-w_opt[0])**2 + (w_svc[1]-w_opt[1])**2+(b_svc - b_opt)**2 > delta and nit < maxit and v-u > delta**4:
    ...
    print(sol.message)
    print(sol.nit)
    print('maxcv is ' + str(attack_norm_constr(x_opt))+'  and  '+str(min(class_constr(x_opt, u, v))))
    if not sol.success :
