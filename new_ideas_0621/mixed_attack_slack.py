...
def classifier_obj(x):
    return sum(x[m+2*n+1:m+3*n+1]) + C*np.dot(x[:m], x[:m])
...
def class_constr_inf_ineq(x, w_prev, ub):
    ...
    for i in range(0, n):
        ...
        ret.append(x[m+n+1+i])
        ret.append(x[m+2*n+1+i])
    ret.append(eps*np.dot(w_prev, w_prev) - np.dot(x[m+1:m+n+1], x[m+1:m+n+1])/n)
    ret.append(np.dot(x[:m], w_prev) - 0.0001)
...
while nit < maxit:
    ...
    cons = [con]
    sol = minimize(objective, np.array(x_opt), constraints=cons, options=options, method='COBYLA')
...
dataset_infected = []
print(class_constr_inf_ineq(x_opt, w_p, ub))
